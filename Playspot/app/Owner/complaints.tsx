// screens/owner/ComplaintsManagement.tsx
...
import { OwnerComplaintsSkeleton } from "../../components/ui/OwnerComplaintsSkeleton";
...
const ComplaintsManagement = () => {
  const [complaintsByOwner, setComplaintsByOwner] = useState<Complaint[]>([]);
  const [complaintsAgainstOwner, setComplaintsAgainstOwner] = useState<PlayerComplaint[]>([]);
  const [loading, setLoading] = useState(true);
  const [showComplaintModal, setShowComplaintModal] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const { approvalStatus } = useOwnerAuth();
  const [activeTab, setActiveTab] = useState<"byOwner" | "againstOwner">("byOwner");

  const [selectedComplaint, setSelectedComplaint] = useState<Complaint | PlayerComplaint | null>(null);
  ...
  { id: "no_show", label: "Player No Show / Late Cancellation" },
  { id: "property_damage", label: "Property Damage" },
  { id: "rule_violation", label: "Rule Violation by Player" },
  ...
  const fetchComplaints = async () => {
    try {
      ...
      try {
        ...
        const ownerComplaintsData = ownerComplaintsSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
          createdAt: doc.data().createdAt?.toDate() || new Date(),
        })) as Complaint[];
        
        setComplaintsByOwner(ownerComplaintsData);
      } catch (error) {
      ...
      try {
        ...
        setComplaintsAgainstOwner(playerComplaintsData);
      } catch (error: any) {
        console.error("Error fetching player complaints:", error);
        
        // Fallback: Get all complaints and filter client-side
        const playerComplaintsRef = collection(db, "player_complaints");
        const playerComplaintsSnapshot = await getDocs(playerComplaintsRef);
        let playerComplaintsData = playerComplaintsSnapshot.docs
          .map((doc) => ({
            id: doc.id,
            ...doc.data(),
            createdAt: doc.data().createdAt?.toDate() || new Date(),
          ...
          .filter((complaint: any) => complaint.ownerName === user.email) as PlayerComplaint[];
  ...
  const submitComplaint = async () => {
    ...
    try {
      ...
      const complaintData = {
        playerId: "", // Will be populated if needed from other systems
        playerName: playerName.trim(),
        playerEmail: playerEmail.trim(),
        groundId: "", // You might want to add ground selection
        groundName: "", // You might want to add ground selection
        type: complaintType,
        description: complaintDescription.trim(),
        status: "pending",
        createdAt: new Date(),
        ownerId: user.uid,
        ownerName: user.displayName || user.email || "Ground Owner",
      };

      await addDoc(collection(db, "owner_complaints"), complaintData);
  ...
  const handleComplaintPress = (complaint: Complaint | PlayerComplaint) => {
  ...
  const renderSimpleComplaintCard = ({ item }: { item: Complaint | PlayerComplaint }) => (
    <TouchableOpacity 
    ...
    >
      <View style={styles.simpleCardHeader}>
        <View style={styles.simpleCardTitleContainer}>
          <View style={[
            ...
            { backgroundColor: activeTab === "byOwner" ? "#EEF2FF" : "#FEF2F2" }
          ]}>
            <Ionicons 
              name={activeTab === "byOwner" ? "person" : "business"} 
              size={20} 
              color={activeTab === "byOwner" ? "#1e293b" : "#EF4444"} 
            />
          </View>
          <View style={styles.simpleCardTextContainer}>
            <Text style={styles.simpleCardTitle} numberOfLines={1}>
              {activeTab === "byOwner" 
                ? `Against: ${(item as Complaint).playerName}`
                : `By: ${(item as PlayerComplaint).playerName}`
              }
            </Text>
      ...
      <View style={styles.simpleCardFooter}>
        ...
        {/* Warning indicator for complaints against owner with admin note */}
        {activeTab === "againstOwner" && (item as PlayerComplaint).adminNote && (
          <View style={styles.warningIndicator}>
            <Ionicons name="warning" size={14} color="#DC2626" />
            <Text style={styles.warningIndicatorText}>Admin Note</Text>
          </View>
        )}
      </View>
    </TouchableOpacity>
      ...
      <View style={styles.detailContainer}>
        ...
        <View style={styles.detailHeader}>
          ...
          <Text style={styles.detailHeaderTitle}>
            {activeTab === "byOwner" ? "Complaint Details" : "Complaint Against You"}
          </Text>
        ...
        >
              ...
              <View style={styles.detailTitleContainer}>
                ...
                <View style={styles.detailTitleText}>
                  <Text style={styles.detailTitle}>
                    {activeTab === "byOwner" 
                      ? `Against: ${(selectedComplaint as Complaint).playerName}`
                      : `By: ${(selectedComplaint as PlayerComplaint).playerName}`
                    }
                  </Text>
          ...
          <View style={styles.detailCard}>
            ...
            <View style={styles.infoGrid}>
              ...
              {activeTab === "againstOwner" && (selectedComplaint as PlayerComplaint).groundName && (
                <View style={styles.infoItem}>
                  <Ionicons name="location" size={18} color="#1e293b" />
                  <View style={styles.infoText}>
                    <Text style={styles.infoLabel}>Ground</Text>
                    <Text style={styles.infoValue}>{(selectedComplaint as PlayerComplaint).groundName}</Text>
                  </View>
                </View>
              )}
              ...
              {activeTab === "byOwner" && (selectedComplaint as Complaint).playerEmail && (
                <View style={styles.infoItem}>
                  <Ionicons name="mail" size={18} color="#1e293b" />
                  <View style={styles.infoText}>
                    <Text style={styles.infoLabel}>Player Email</Text>
                    <Text style={styles.infoValue}>{(selectedComplaint as Complaint).playerEmail}</Text>
                  </View>
                </View>
              )}
            </View>
          ...
          <View style={styles.detailCard}>
            <Text style={styles.sectionTitle}>
              {activeTab === "byOwner" ? "Complaint Description" : "Complaint Details"}
            </Text>
            <Text style={styles.descriptionText}>{selectedComplaint.description}</Text>
          </View>

          {/* Admin Note for complaints against owner */}
          {activeTab === "againstOwner" && (selectedComplaint as PlayerComplaint).adminNote && (
            <View style={[styles.detailCard, styles.infoCard]}>
              <View style={styles.infoHeader}>
                <Ionicons name="information-circle" size={24} color="#F59E0B" />
                <Text style={styles.infoCardTitle}>Admin Resolution Note</Text>
              </View>
              <Text style={styles.infoCardText}>{(selectedComplaint as PlayerComplaint).adminNote}</Text>
              {(selectedComplaint as PlayerComplaint).resolvedAt && (
                <Text style={styles.resolvedDate}>
                  Resolved on: {formatDate((selectedComplaint as PlayerComplaint).resolvedAt!)}
                </Text>
              )}
            </View>
          )}
          ...
          {selectedComplaint.status === "resolved" && !(selectedComplaint as PlayerComplaint).adminNote && (
  ...
  const renderListView = () => (
    <View style={styles.listContainer}>
      <HeaderComponent
        ...
        subtitle="Handle and resolve user complaints quickly"
      ...
      <View style={styles.content}>
        ...
        <View style={styles.tabContainer}>
          <TouchableOpacity 
            style={[styles.tab, activeTab === "byOwner" && styles.activeTab]}
            onPress={() => setActiveTab("byOwner")}
          >
            <Ionicons 
              name="arrow-up-circle" 
              size={20} 
              color={activeTab === "byOwner" ? "#FFFFFF" : "#1e293b"} 
            />
            <Text style={[styles.tabText, activeTab === "byOwner" && styles.activeTabText]}>
              Your Complaints
            </Text>
            {complaintsByOwner.length > 0 && (
              <View style={styles.badge}>
                <Text style={styles.badgeText}>{complaintsByOwner.length}</Text>
              </View>
            )}
          </TouchableOpacity>
          
          <TouchableOpacity 
            style={[styles.tab, activeTab === "againstOwner" && styles.activeTab]}
            onPress={() => setActiveTab("againstOwner")}
          >
            <Ionicons 
              name="arrow-down-circle" 
              size={20} 
              color={activeTab === "againstOwner" ? "#FFFFFF" : "#EF4444"} 
            />
            <Text style={[styles.tabText, activeTab === "againstOwner" && styles.activeTabText]}>
              Against You
            </Text>
            {complaintsAgainstOwner.length > 0 && (
              <View style={[styles.badge, styles.badgeWarning]}>
                <Text style={styles.badgeText}>{complaintsAgainstOwner.length}</Text>
              </View>
            )}
          </TouchableOpacity>
        </View>
        ...
        {/* Complaints List */}
        <FlatList
          data={activeTab === "byOwner" ? complaintsByOwner : complaintsAgainstOwner}
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubtitle}>
                {activeTab === "byOwner"
                  ? "You haven't filed any complaints against players yet."
                  : "You have no complaints from players at this time."
                }
              </Text>
              {activeTab === "byOwner" && (
                <TouchableOpacity
  ...
  if (loading) {
    return <OwnerComplaintsSkeleton />;
  }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.approvalContainer}>
          ...
          <Text style={styles.notApprovedText}>
            Your account is pending admin approval. You cannot manage complaints yet.
          </Text>
        </View>
      </View>
    );
